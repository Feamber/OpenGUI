...
OGUI::RestyleDamage OGUI::ComputedStyle::ApplyTransitionProperties(const ComputedStyle& src, const ComputedStyle& dst, const gsl::span<TransitionProperty>& props)
{
    RestyleDamage damage = RestyleDamage::None;
    auto& registry = GetRegistry();
    for(auto& desc : registry.descriptions)
        damage |= desc.ApplyTransitionProperties(*this, src, dst, props);
    return damage;
}
