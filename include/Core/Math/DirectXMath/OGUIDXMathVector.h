...
namespace OGUI::math::__vector
{
    ...
    FORCEINLINE VectorRegister load(const OGUI::span<const float, 4> vec)
    {
        return DirectX::XMLoadFloat4(reinterpret_cast<const DirectX::XMFLOAT4*>(vec.data()));
    }

    FORCEINLINE VectorRegister load_uint1(const OGUI::span<const uint32, 1> vec)
    {
        return vector_register(vec[0], vec[1], vec[0], vec[1]);
    }
	
	FORCEINLINE VectorRegister load_float1(const OGUI::span<const float, 1> vec)
    {
        return vector_register(vec[0], vec[1], vec[0], vec[1]);
    }

	FORCEINLINE VectorRegister load_float2(const OGUI::span<const float, 2> vec)
    {
        return vector_register(vec[0], vec[1], vec[0], vec[1]);
    }

    FORCEINLINE VectorRegister load_uint2(const OGUI::span<const uint32, 2> vec)
    {
        return vector_register(vec[0], vec[1], vec[0], vec[1]);
    }
	
    FORCEINLINE VectorRegister load_float3_w0(const OGUI::span<const float, 3> vec)
    {
        return vector_register(vec[0], vec[1], vec[2], 0.f);
    }
	
    FORCEINLINE VectorRegister load_float3_w1(const OGUI::span<const float, 3> vec)
    {
        return vector_register(vec[0], vec[1], vec[2], 1.f);
    }

    FORCEINLINE VectorRegister load_uint3_w0(const OGUI::span<const uint32, 3> vec)
    {
        return vector_register(vec[0], vec[1], vec[2], 0u);
    }

    FORCEINLINE VectorRegister load_uint3_w1(const OGUI::span<const uint32, 3> vec)
    {
        return vector_register(vec[0], vec[1], vec[2], 1u);
    }

    FORCEINLINE VectorRegister load_aligned(const OGUI::span<const float, 4> vec)
    {
        return DirectX::XMLoadFloat4A(reinterpret_cast<const DirectX::XMFLOAT4A*>(vec.data()));
    }

    FORCEINLINE void store_aligned(OGUI::span<float, 4> target, const VectorRegister vector)
    {
        return DirectX::XMStoreFloat4A(reinterpret_cast<DirectX::XMFLOAT4A*>(target.data()), vector);
    }

    FORCEINLINE void store(OGUI::span<float, 4> target, const VectorRegister vector)
    {
        return DirectX::XMStoreFloat4(reinterpret_cast<DirectX::XMFLOAT4*>(target.data()), vector);
    }

    FORCEINLINE void store_float3(OGUI::span<float, 3> target, const VectorRegister vector)
    {
        return DirectX::XMStoreFloat3(reinterpret_cast<DirectX::XMFLOAT3*>(target.data()), vector);
    }

    FORCEINLINE void store_float1(OGUI::span<float, 3> target, const VectorRegister vector)
    {
