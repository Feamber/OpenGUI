...
struct _Any_big_RTTI { // Hand-rolled vtable for types that must be heap allocated in an any
    using _Destroy_fn = void __cdecl(void*) _NOEXCEPT_FNPTR;
    ...
    template <class _Ty>
    [[nodiscard]] static void* __cdecl _Copy_impl(const void* const _Source) {
...
struct _Any_small_RTTI { // Hand-rolled vtable for nontrivial types that can be stored internally in an any
    ...
    using _Move_fn    = void __cdecl(void*, void*) _NOEXCEPT_FNPTR;
...
public:
    ...
    any(const any& _That) {
        ...
        default:
            std::memcpy(_Storage._TrivialData, _That._Storage._TrivialData, sizeof(_Storage._TrivialData));
            break;
        }
    }
    ...
    any(_ValueType&& _Value) { // initialize with _Value
        _Emplace<decay_t<_ValueType>>(std::forward<_ValueType>(_Value));
    }

    any(const void* _Value,  const type_info* const _Info){
        // using the _Trivial representation
        const void*& _Obj = reinterpret_cast<const void*&>(_Storage._TrivialData);
        _Construct_in_place(_Obj, _Value);
        _Storage._TypeData =
            reinterpret_cast<uintptr_t>(_Info) | static_cast<uintptr_t>(_Any_representation::_Trivial);
    }
    ...
    [[nodiscard]] const _Decayed* _Cast() const noexcept {
    ...
    template <class _Decayed>
    [[nodiscard]] _Decayed* _Cast() noexcept { // if *this contains a value of type _Decayed, return a pointer to it
...
private:
    ...
    [[nodiscard]] _Any_representation _Rep() const noexcept { // extract the representation format from _TypeData
        return static_cast<_Any_representation>(_Storage._TypeData & _Rep_mask);
    }
    [[nodiscard]] const type_info* _TypeInfo() const noexcept { // extract the type_info from _TypeData
...
template <class _ValueType, class _Elem, class... _Types>
[[nodiscard]] any make_any(initializer_list<_Elem> _Ilist, _Types&&... _Args) {
...
[[nodiscard]] const _ValueType* any_cast(const any* const _Any) noexcept {
...
template <class _ValueType>
[[nodiscard]] _ValueType* any_cast(any* const _Any) noexcept {
    // retrieve a pointer to the _ValueType contained in _Any, or null
    static_assert(!is_void_v<_ValueType>, "std::any cannot contain void.");
...
template <class _Ty>
[[nodiscard]] remove_cv_t<_Ty> any_cast(const any& _Any) {
...
template <class _Ty>
[[nodiscard]] remove_cv_t<_Ty> any_cast(any& _Any) {
...
[[nodiscard]] remove_cv_t<_Ty> any_cast(any&& _Any) {
    ...
    using any = any_detail::any;
    using any_detail::any_cast;
    using any_detail::make_any;
    using any_detail::swap;
}