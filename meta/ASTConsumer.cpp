...
void meta::ASTConsumer::HandleTranslationUnit(ASTContext &ctx)
{
    ...
    for (clang::DeclContext::decl_iterator i = tuDecl->decls_begin(); i != tuDecl->decls_end(); ++i)
    {
...
void meta::ASTConsumer::HandleDecl(clang::NamedDecl* decl, std::vector<std::string>& attrStack, ParseBehavior behavior, Record* record)
{
    ...
    clang::NamedDecl* attrDecl = decl;
    {
        ...
        auto iter = db.parsed.find(ident);
        if(iter != db.parsed.end())
            return;
        db.parsed.insert(ident);
    }
    ...
    case (clang::Decl::CXXRecord):
        {
            ...
            if(!recordDecl || recordDecl->isThisDeclarationADefinition() == clang::VarDecl::DeclarationOnly)
            {
    ...
    case (clang::Decl::CXXMethod):
        {
            clang::FunctionDecl* functionDecl = llvm::dyn_cast<clang::FunctionDecl>(decl);
            if(!functionDecl)
                return;
